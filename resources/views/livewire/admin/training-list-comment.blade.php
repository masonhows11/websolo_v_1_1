        ...
        <div class="row d-flex flex-column list-unapproved-comments">
            <div class="col-md-4 post-card-section">
                <div class="row">
                    <div class="col">
                        <div class="card">
                            <img src="{{ asset('/storage/training/' . $training->image) }}" class="img-fluid card-img-top"
                                 alt="article-image">
                            <div class="card-body mt-3 comment-post-title">
                                <h4 class="card-title">{{ $training->user->name }}</h4>
                                <h5 class="card-title">{{ $training->title_persian }}
                                    - {{ $training->title_english }}</h5>
                                <p>{!! substr($training->short_description, 0, 200) !!}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if ($training->comments->count() != null)
                    ...
                    <div class="accordion-item mt-5">
                        <div class="accordion-header border border-2" id="headingOne">
                            <bottun class="accordion-button" data-bs-toggle="collapse"
                                    data-bs-target="#collapse-{{ $training->id }}" aria-expanded="true"
                                    aria-controls="collapseOne">
                            </bottun>
                        </div>
                        <div id="collapse-{{ $training->id }}"
                             class="accordion-collapse collapse {{ $training->comments->count() >= 1 ? 'show' : '' }}"
            ...
            @else
                <div class="col-md-4">
                    <div class="alert alert-light mt-5 border border-3">
                        <strong> برای این آموزش دیدگاهی ثبت نشده.</strong>
                    </div>
                </div>
            @endif
        </div>
